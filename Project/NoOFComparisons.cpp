...
void NoOFCompare::Selection_sort(List& arr, long long int &count_compare){
    int n = arr.size(); 
...
void NoOFCompare::Insertion_sort(List& arr, long long int &count_compare){
	int n=arr.size();
...
void NoOFCompare::Bubble_sort(List& arr, long long int &count_compare){
	int n = arr.size();
	for (int i = 0; ++count_compare && i < n - 1; i++)
...
void NoOFCompare::Shaker_sort(List& arr, long long int &count_compare){
	int n= arr.size();
	int i, j, k;
   	for(i = 0; i < n;) {
      	for(j = i+1; j < n; j++) {
...
void NoOFCompare::Shell_sort(List& arr, long long int &count_compare){}

void NoOFCompare::heapRebuild(int start, List& arr, int n, long long int &count_compare){
...
void NoOFCompare::Heap_sort(List& arr, long long int &count_compare){
	int n=arr.size();
	// Build initial heap
...
void NoOFCompare::merge(List& arr, int first, int mid, int last, long long int &count_compare){
...
void NoOFCompare::Merge_sort(List& arr, int first, int last, long long int &count_compare){
	if (first < last) {
...
void NoOFCompare::Quick_sort(List& arr, int first, int last, long long int &count_compare){
...
void NoOFCompare::Counting_sort(List& arr, int u, long long int &count_compare){}

void NoOFCompare::Radix_sort(List& arr, long long int &count_compare){
    int n = arr.size(); 
...
void NoOFCompare::Flash_sort(List& arr, long long int &count_compare){
